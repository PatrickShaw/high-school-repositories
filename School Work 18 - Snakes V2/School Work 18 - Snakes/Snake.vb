...
Public NotInheritable Class Snake
            ...
            For i As Long = 0 To coordsToBePassed.Count - 1
                Select Case CheckCoordinatesFilled(coordsToBePassed(i))
                    ...
                    Case CollisionType.none
                        space += 1
                        If debug Then
                            graphPen.Color = ColourValue(currentPass * 2, 2)
                            graph.FillRectangle(graphPen, coordsToBePassed(i).x * boxSize, coordsToBePassed(i).y * boxSize, boxSize, boxSize)
                        End If
    ...
    Private Function FindBestDOption(options As List(Of Direction)) As Direction
        ...
        For i = 0 To options.Count - 1
            ...
            End If
            'If i = 0 Then CalcSpace(300, snakeBlocks.First.coord + delta, True)
            dNew = options(i)
            DetermineDelta()
    ...
    Public Sub Update()
        If dead Then Return
        If invicibilityTime > 0 Then invicibilityTime -= 1 * frmSnake.tmrGame.Interval Else invicibilityTime = 0
        If isAI = False Then
            ...
            End If
        End If
        If (moveStopWatch.ElapsedMilliseconds < (moveSpeed / speedFactor / globalSpeed)) Then Return
        beenUpdated = True
        If isAI Then AI()
        If dNew = dOld Then
    ...
    Public Sub Move()
        ...
        If growthQueue < 0 Then
            If snakeBlocks.Count() >= 2 Then
                SpawnFood(New EdibleSnake(snakeBlocks.Last.colour), snakeBlocks.Last.coord)
    ...
    Public Function HighlightColour(colourDefault As Color) As Color
        Return Color.FromArgb(calcHighVal(colourDefault.R), calcHighVal(colourDefault.G), calcHighVal(colourDefault.B))
    End Function
    ...
    Public Function ColourValue(input As Long, snakeNo As Long) As Color
        ...
        Select Case snakeNo
            ...
            Case 2
                CalculateColour(input + 800, B, G, R)
            Case 3
                CalculateColour(input + 130, G, B, R)
            Case 4
                CalculateColour(input + 500, R, B, G)
            Case 5
